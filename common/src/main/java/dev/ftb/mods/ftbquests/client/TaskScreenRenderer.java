...
public class TaskScreenRenderer implements BlockEntityRenderer<TaskScreenBlockEntity, TaskScreenRenderState> {
    ...
    @Override
    public void extractRenderState(TaskScreenBlockEntity blockEntity, TaskScreenRenderState blockEntityRenderState, float f, Vec3 vec3, ModelFeatureRenderer.@Nullable CrumblingOverlay crumblingOverlay) {
        BlockEntityRenderer.super.extractRenderState(blockEntity, blockEntityRenderState, f, vec3, crumblingOverlay);
        blockEntityRenderState.isInputOnly = blockEntity.isInputOnly();
        blockEntityRenderState.teamId = blockEntity.getTeamId();
    ...
    @Override
    public void submit(TaskScreenRenderState taskScreen, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState cameraRenderState) {
        if (!ClientQuestFile.exists() || !(taskScreen.blockState.getBlock() instanceof TaskScreenBlock taskScreenBlock)) return;

        TeamData data = ClientQuestFile.INSTANCE.getNullableTeamData(taskScreen.teamId);
        Task task = taskScreen.task;
        if (task == null || data == null) return;

        poseStack.pushPose();

        poseStack.translate(0.5D, 0.5D, 0.5D);
        float rotation = taskScreen.blockState.getValue(WallSignBlock.FACING).toYRot() + 180f;
...
//        }));
    ...
    private void drawTaskIcon(TaskScreenRenderState taskScreen, TeamData data, final Icon<?> icon, PoseStack poseStack, SubmitNodeCollector buffer) {
        poseStack.pushPose();
        poseStack.scale(1 / 16f, 1 / 16f, 1 / 16f);

        buffer.submitCustomGeometry(poseStack, RenderTypes.text(AtlasIds.BLOCKS), ((pose, consumer) -> {
            Task task = taskScreen.task;
            long progress = data.getProgress(task);

            Icon<?> currentIcon = icon;
            if (icon instanceof AnimatedIcon anim) {
                currentIcon = anim.getList().get((int)(System.currentTimeMillis() / 1000L % (long) anim.getList().size()));
            }
            ...
            if (task instanceof FluidTask fluidTask && fluidTask.getIcon() instanceof AtlasSpriteIcon as && FTBQuestsClientEventHandler.tankSprite != null) {
                TextureAtlasSprite sprite = Minecraft.getInstance().getAtlasManager().getAtlasOrThrow(AtlasIds.BLOCKS).getSprite(as.getSpriteId());
                ...
                if (progress > 0L) {
                    ...
                    RenderUtil.create(poseStack, consumer, -8f, -8f)
                            .withColor(ClientFluidStackHooks.getColor(fluidTask.getFluid()) | 0xFF000000)
            ...
            } else if (currentIcon instanceof ItemIcon itemIcon) {
                // TODO a flat 3d-render (GUI style) would be better here
                poseStack.scale(16f, 16f, 16f);
                // TODO: @since 21.11: Impl new rendering system
...
//                        OverlayTexture.NO_OVERLAY, poseStack, buffer, Minecraft.getInstance().level, 0);
            } else if (currentIcon instanceof AtlasSpriteIcon spriteIcon) {
                var sprite = Minecraft.getInstance().getAtlasManager().getAtlasOrThrow(AtlasIds.BLOCKS).getSprite(spriteIcon.getSpriteId());
                RenderUtil.create(poseStack, consumer, -8f, -8f)
